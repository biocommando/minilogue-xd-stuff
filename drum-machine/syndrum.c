...
void read_params(FILE *f, const char *section)
{
    ...
    while (!feof(f))
    {
        ...
        else printf("Could not parse config line '%s'\n", buf);
    }
...
void params_to_code()
{
    if (params[SD_FREQ_WF_IDX] == 0)
        ...
        printf("SD_PACK_FREQ_WF(%s, %d)\n", waveform == (int)OSC_TRIANGLE
            ? "OSC_TRIANGLE" :
            (waveform == (int)OSC_SQUARE ? "OSC_SQUARE" : "OSC_SAW"),
            freq);
...
{
    if (argc < 3) { puts("Required parameters: input path, output path"); return 1; }
    FILE *f = fopen(argv[1], "r");
    if (!f) { puts("Error opening output file"); return 1; }
